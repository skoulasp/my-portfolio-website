...
function Footer({ handleNavigateToRoute, delayRef }) {
    ...
    useEffect(() => {
        setShowTopBtn(
            location.pathname !== "/privacy-policy" && location.pathname !== "/terms-of-use" && location.pathname !== "/photo-credits"
        );
    }, [location]);
    ...
    const handleToTopBtnClick = (e) => {
        ...
        if (targetSection) {
            ...
            let scrollTimeout;
            const hadleSrollTimeout = () => {
                clearTimeout(scrollTimeout);
                scrollTimeout = setTimeout(scrollEndListener, 200);
            };
            if (delayRef.current) {
                window.addEventListener("scroll", hadleSrollTimeout);
            }
            const scrollEndListener = () => {
                delayRef.current = false;
                clearTimeout(scrollTimeout);
                window.removeEventListener("scroll", hadleSrollTimeout);
                return;
            };
        }
    };
            ...
            <div className="footer-content">
                <div className="legal-links">
                    <Link
                        ...
                        className={location.pathname === "/terms-of-use" ? "highlight" : ""}
                    >
                        {data.legal.terms.title}
                    </Link>
                    <Link
                        to="/privacy-policy"
                        onClick={handleNavigateToRoute}
                        className={location.pathname === "/privacy-policy" ? "highlight" : ""}
                    >
                        {data.legal.privacy.title}
                    </Link>{" "}
                    <Link
                        to="/photo-credits"
                        onClick={handleNavigateToRoute}
                        className={location.pathname === "/photo-credits" ? "highlight" : ""}
                    >
                        {data.legal.credits.title}
                    </Link>
                </div>
                {showTopBtn && (
                    <div className="top-btn">
                        <a onClick={handleToTopBtnClick} href="/#home" className="social-link">
                            🔺 {data.footer.backbtn} 🔺
                        </a>
                    </div>
                )}
                ...
                <div className="copyright">
                    ©{displayYear} {data.footer.copyright}
                </div>

                <div className="social-media">
                    <a href="https://www.linkedin.com/in/skoulasp/" target="_blank" rel="noreferrer" className="contact-item social">
